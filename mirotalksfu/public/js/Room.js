...
function initClient() {
    ...
    if (!isMobileDevice) {
        ...
        setTippy(
            ...
            'Если включено, при разговоре участника его видео будет увеличено',
            'right'
        );
        setTippy(
            'switchNoiseSuppression',
            'Если включено, звук будет обработан для уменьшения фонового шума',
            'right'
        );
        setTippy(
            'switchPushToTalk',
            'Если включено, при нажатии пробела микрофон включится, при отпускании - выключится',
            'right'
        );
        ...
        setTippy(
            'switchBroadcasting',
            'Трансляция - это распространение аудио или видео контента большой аудитории',
            'right'
        );
        setTippy(
            'switchLobby',
            'Режим лобби позволяет защитить встречу, разрешая вход только после одобрения модератора',
            'right'
        );
        setTippy('initVideoAudioRefreshButton', 'Обновить аудио/видео устройства', 'top');
        setTippy(
            'screenOptimizationLabel',
            'Детализация: Для высокой четкости (демонстрация текста/графики)<br />Движение: Для высокой частоты кадров (видео, игры)',
            'right',
            true
        );
        setTippy('switchPitchBar', 'Переключить индикатор громкости', 'right');
        setTippy('switchSounds', 'Переключить звуковые уведомления', 'right');
        setTippy('switchShare', "Показать окно 'Поделиться комнатой' при входе", 'right');
        setTippy('switchKeepButtonsVisible', 'Кнопки всегда видны', 'right');
        setTippy('roomId', 'Название комнаты (нажмите для копирования)', 'right');
        setTippy('sessionTime', 'Время сессии', 'right');
        setTippy('recordingImage', 'Переключить запись', 'right');
        setTippy(
            'switchHostOnlyRecording',
            'Только хост (ведущий) может записывать встречу',
            'right'
        );
        setTippy('refreshVideoFiles', 'Обновить', 'left');
        setTippy('switchServerRecording', 'Запись будет сохранена на сервере, а не локально', 'right');
        ...
        setTippy('whiteboardRedoBtn', 'Повторить', 'bottom');
        ...
        setTippy('showChatOnMsg', 'Показать чат при новом сообщении', 'bottom');
        ...
        setTippy('chatMarkdownButton', 'Разметка', 'top');
        setTippy('chatCloseButton', 'Закрыть', 'bottom');
        setTippy('chatTogglePin', 'Закрепить', 'bottom');
        setTippy('chatHideParticipantsList', 'Скрыть', 'bottom');
        setTippy('chatShowParticipantsList', 'Список участников', 'bottom');
        setTippy('chatMaxButton', 'Развернуть', 'bottom');
        setTippy('chatMinButton', 'Свернуть', 'bottom');
        setTippy('pollTogglePin', 'Закрепить', 'bottom');
        ...
        setTippy('editorLockBtn', 'Заблокировать редактор', 'bottom');
        setTippy('editorUnlockBtn', 'Разблокировать редактор', 'bottom');
        setTippy('editorTogglePin', 'Закрепить', 'bottom');
        setTippy('editorUndoBtn', 'Отменить', 'bottom');
        setTippy('editorRedoBtn', 'Повторить', 'bottom');
        ...
        setTippy('participantsSaveBtn', 'Сохранить информацию об участниках', 'bottom');
        setTippy('participantsRaiseHandBtn', 'Поднятые руки', 'bottom');
        setTippy('participantsUnreadMessagesBtn', 'Непрочитанные сообщения', 'bottom');
        setTippy('transcriptionCloseBtn', 'Закрыть', 'bottom');
        setTippy('transcriptionTogglePinBtn', 'Закрепить', 'bottom');
        setTippy('transcriptionMaxBtn', 'Развернуть', 'bottom');
        setTippy('transcriptionMinBtn', 'Свернуть', 'bottom');
        setTippy('transcriptionSpeechStatus', 'Статус', 'bottom');
        setTippy('transcriptShowOnMsg', 'Показать транскрипцию при новом сообщении', 'bottom');
        setTippy('transcriptionSpeechStart', 'Начать транскрипцию', 'top');
        setTippy('transcriptionSpeechStop', 'Остановить транскрипцию', 'top');
    }
    setupWhiteboard();
...
function refreshMainButtonsToolTipPlacement() {
    if (!isMobileDevice) {
        ...
        setTippy('fullScreenButton', 'Полноэкранный режим', placement);
        ...
        // Bottom buttons
        setTippy('toggleExtraButton', 'Дополнительные кнопки', bPlacement);
        ...
        setTippy('swapCameraButton', 'Сменить камеру', bPlacement);
        setTippy('startScreenButton', 'Демонстрация экрана', bPlacement);
        setTippy('stopScreenButton', 'Остановить демонстрацию', bPlacement);
        setTippy('raiseHandButton', 'Поднять руку', bPlacement);
        setTippy('lowerHandButton', 'Опустить руку', bPlacement);
        setTippy('chatButton', 'Открыть чат', bPlacement);
    ...
    Swal.fire({
        ...
        inputValidator: (name) => {
            if (isVideoAllowed && !isInitVideoLoaded) {
                return 'Пожалуйста, подождите инициализацию видео...';
            }
            if (!name) return 'Пожалуйста, введите имя';
            const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(name);
            if ((isEmail && name.length > 254) || (!isEmail && name.length > 32)) {
                return isEmail ? 'Email должен быть максимум 254 символа' : 'Имя должно быть максимум 32 символа';
            }
            name = filterXSS(name);
...
async function shareRoom(useNavigator = false) {
    ...
    function share() {
        ...
        Swal.fire({
            ...
            <br/>
            <p style="background:transparent; color:rgb(8, 189, 89);">Присоединитесь с мобильного устройства</p>
            <p style="background:transparent; color:white; font-family: Arial, Helvetica, sans-serif;">Не нужны приложения, просто наведите камеру телефона на QR-код или пригласите кого-то, отправив следующую ссылку</p>
...
function copyRoomURL() {
    ...
    userLog('info', 'Ссылка скопирована в буфер обмена', 'top-end');
}
...
function copyToClipboard(txt, showTxt = true) {
    ...
    document.body.removeChild(tmpInput);
    showTxt
        ? userLog('info', `${txt} скопировано в буфер обмена`, 'top-end')
...
function setButtonsInit() {
    if (!isMobileDevice) {
        setTippy('initAudioButton', 'Переключить звук', 'top');
        setTippy('initVideoButton', 'Переключить видео', 'top');
        setTippy('initAudioVideoButton', 'Переключить звук и видео', 'top');
        setTippy('initStartScreenButton', 'Демонстрация экрана', 'top');
        setTippy('initStopScreenButton', 'Остановить демонстрацию', 'top');
        setTippy('initVideoMirrorButton', 'Отзеркалить видео', 'top');
...
function setParticipantsTippy(peers) {
    //
    if (!isMobileDevice) {
        setTippy('muteAllButton', 'Выключить звук всем участникам', 'top');
        setTippy('hideAllButton', 'Скрыть всех участников', 'top');
        setTippy('stopAllButton', 'Остановить демонстрацию экрана для всех', 'top');
        //
        for (let peer of Array.from(peers.keys())) {
            const peer_info = peers.get(peer).peer_info;
            const peer_id = peer_info.peer_id;
            ...
            if (peerAudioBtn) setTippy(peerAudioBtn.id, 'Выключить', 'top');
                ...
                ${
                    BRAND.about?.html && BRAND.about.html.trim() !== ''
                        ...
                        : `
                            ...
                            <br /><br />
                            <hr />
                            <span>&copy; 2025 ExCall, все права защищены</span>
                            <hr />
                        `
                }
            </div>
